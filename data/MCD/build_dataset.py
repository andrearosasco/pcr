...
def read_yaml_file(file_path):
    with open(file_path, 'r') as stream:
        try:
            return yaml.load(stream, Loader=BaseLoader)
...
def get_train_and_test_model_names(dataset_seed, use_default_train_test_split):
    if use_default_train_test_split:
        model_names = read_yaml_file(
            "data/MCD/grasping_train_test_split.yaml")
        ...
        model_names = read_yaml_file("objects.yaml")
...
def build(dataset_type, path_to_data, dataset_seed,
    ...
    dataset_file = dataset_type + "_dataset.json"
    create_file(dataset_folder, dataset_file)
...
def verify_example(x_filepath, y_filepath, patch_size):
    ...
    x = np.array(read_point_cloud(x_filepath).points).astype(int)
    x_mask = (x[:, 0], x[:, 1], x[:, 2])
    y = np.array(read_point_cloud(y_filepath).points).astype(int)
    y_mask = (y[:, 0], y[:, 1], y[:, 2])
...
def verify(dataset_file_path):
    ...
    Success = True
    problems = []
    dataset_type = json.load(open(dataset_file_path, 'r'))
    for key in [
