...
#####################################################
model = HyperNetwork.load_from_checkpoint('checkpoint/absolute_best', config=ModelConfig)
model = model.to('cuda')
model.eval()
...
for i in range(100):
    ...
    samples = create_3d_grid(batch_size=model_input.shape[0], step=0.1).to(TrainConfig.device)
    ...
    ##################################################
    start = time.time()
    refined_pred = torch.tensor(samples[:, prediction >= 0.0, :].cpu().detach().numpy(), device=TrainConfig.device,
                                requires_grad=True)
    refined_pred_0 = copy.deepcopy(refined_pred.detach())

    loss_function = BCEWithLogitsLoss(reduction='mean')
    optim = Adam([refined_pred], lr=0.01)
