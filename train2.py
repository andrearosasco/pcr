...
def main():
    ...
    if Config.Eval.wandb:
        if resume:
            wandb.init(project=project, id=id, resume='must', config=Config.to_dict(), reinit=True,
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor='val_models/chamfer',
        dirpath='checkpoint',
        filename='epoch{epoch:02d}-cd{val_models/chamfer:.2f}',
        mode='min',
        save_last=True,
