from utils.triangle_hash import TriangleHash
import numpy as np
...
class MeshIntersector:
    def __init__(self, mesh, resolution=512):
        # Triangles contain for each triangle, the coordinates of the points that compose it
        # TODO MY PART
        tri_v_idx = np.array(mesh.triangles)
        vertices = np.array(mesh.vertices)
        triangles = vertices[tri_v_idx]
        # TODO END MY PART
        # triangles = mesh.vertices[mesh.faces].astype(np.float64) TODO READD
        # TODO END THEIR PART
        n_tri = triangles.shape[0]

        self.resolution = resolution
        # Extract the minimum and maximum x, y and z
