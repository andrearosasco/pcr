...
class ShapeNet(data.Dataset):
    def __init__(self, config, mode):
        ...
        print("Found ", self.n_samples, " instances from " + config.dataset_path + os.sep + f'{self.mode}.txt')
    ...
    def __getitem__(self, idx):  # Must return complete, imp_x and impl_y
        ...
        partial_pcd, _ = misc.seprate_point_cloud(complete_pcd, DataConfig().N_POINTS,
                                              [int(DataConfig().N_POINTS * 1 / 4),
                                               int(DataConfig().N_POINTS * 3 / 4)],
                                              fixed_points=None)
        # TODO Start Remove
        # img_width = 640
        # img_height = 480
